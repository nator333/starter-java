package com.twilio.starter.utils;
...
public class FileAccessUtil {
     ...
     *
     * @param src ファイルのパス、Jar内のファイルへのURL、リソースのパス
     * @param dst コピー先のパス
     * @return コピー先のPath
     */
     ...
     *
     * @param uri コピー元となるjar(zip)内のファイルを指定したURI
     * @param dst コピー先のパス
     * @return コピー先のPath
     * @throws IOException
     */
     ...
     *
     * @param uri Jar(zip)内のファイルを指定したURI
     * @throws IOException
     */
    private byte[] getReadAllBytesInJar(URI uri) throws IOException {
        ...
        env.put("create", "true");
        try (FileSystem jarfs = FileSystems.newFileSystem(URI.create(uri.toString().split("!")[0]), env)) {
            Path path = jarfs.getPath(uri.toString().split("!")[1]);
    ...
    public Path getPath(URI uri) throws IOException {
        ...
        env.put("create", "true");
        try (FileSystem jarfs = FileSystems.newFileSystem(URI.create(uri.toString().split("!")[0]), env)) {
            return jarfs.getPath(uri.toString().split("!")[1]);
        }
    }
     ...
     *
     * @param src ファイルシステムのパス、jar()内ファイルのURL、リソースのパスのいずれか
     * @return
     */
     ...
     *
     * @param src jar(zip)内のファイルを示すURL
     ...
     * リソースのURLを取得します。
     *
     * @param src リソース内のパスを指定します。 <br>
     *            例) /samlpe/sample.txt<br>
     *            例) sample.txtt
     * @return リソースのURL {@code null} ファイルが見つからなかった場合
     */
    public URL getURLInResource(String src) {
